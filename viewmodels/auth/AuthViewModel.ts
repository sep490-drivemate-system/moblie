...
export class AuthViewModel extends BaseViewModel<AuthState> {
    ...
    // Auto navigation khi đã authenticated
        ...
        await this.executeAsync(
            async () => {
                ...
                if (result?.data?.accessToken) {
                    await AsyncStorage.setItem(ENV.STORAGE_KEYS.ACCESS_TOKEN, result.data.accessToken);
                    await AsyncStorage.setItem(ENV.STORAGE_KEYS.REFRESH_TOKEN, result.data.refreshToken);
                }
                ...
                // Không cần navigate ở đây, _layout.tsx sẽ handle
            },
            () => {
    ...
    async register(): Promise<void> {
        await this.executeAsync(
            async () => {
                ...
                const result = await this.dispatch(signUp(currentState.registerFormData)).unwrap();
    ...
    simulateAuthenticatedUser(): void {
        ...
        console.log('✅ TEST: User set as authenticated - Check _layout.tsx navigation');
    }
    ...
    simulateAuthenticatedOnly(): void {
        ...
        console.log('✅ TEST: Only isAuthenticated = true - Check _layout.tsx navigation');
    }
